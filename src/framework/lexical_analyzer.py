...
class LexicalAnalyzer():
    ...
    def initialize_symbol_table(self):
        ...
        """
        return LexicalAnalyzer.SymbolTable()
    ...
    def generate(self):
        ...
        try:
            if not self.dfas:
                self.application.error("Nenhum DFA foi adicionado para gerar o analisador léxico.")
    ...
    def unite_by_epsilon(self):
        ...
        for key, dfa_orig in self.dfas.items():
            if not dfa_orig or not hasattr(dfa_orig, 'states'):
                self.application.error(f"DFA inválido para a chave '{key}'. Pulando.")
                continue

            prefix = f"{key}{self.DEFAULT_PREFIX_SEPARATOR}"

            state_mapping = {state: prefix + str(state) for state in dfa_orig.states}
            ...
            if (new_start_state, NonDeterministicFiniteAutomata.EPSILON) not in new_transitions:
                new_transitions[(new_start_state, NonDeterministicFiniteAutomata.EPSILON)] = set()
            new_transitions[(new_start_state, NonDeterministicFiniteAutomata.EPSILON)].add(renamed_dfa_start_state)
        ...
        while unmarked_dfa_states:
            ...
            for nfa_state_name in current_dfa_state_T:
                if nfa_state_name in nfa.accept_states:
                    token_key = nfa_state_name.split(self.DEFAULT_PREFIX_SEPARATOR, 1)[0]
                    if token_key in self.dfas:
                        priority = token_type_priority.get(token_key, float('inf'))
            ...
            for symbol in nfa.alphabet: 
                ...
                for q_nfa in current_dfa_state_T:
                    nfa_states_after_move.update(nfa.transitions.get((q_nfa, symbol), set()))
                
                if not nfa_states_after_move:
                    continue

                target_dfa_state_U = frozenset(nfa._epsilon_closure(nfa_states_after_move))
                if not target_dfa_state_U: 
                    continue

                dfa_transitions[(current_dfa_state_T, symbol)] = target_dfa_state_U

                if target_dfa_state_U not in dfa_states:
                    dfa_states.add(target_dfa_state_U)
                    unmarked_dfa_states.append(target_dfa_state_U)
        
        dfa_actual_accept_states = set(self.dfa_accept_state_to_token_type_map.keys())
        
        dfa_alphabet = {s for s in nfa.alphabet if s != NonDeterministicFiniteAutomata.EPSILON}
    ...
    def process(self, input_stream):
        ...
        if self.has_errors or not self.dfa:
            self.application.error("Analisador léxico não foi gerado ou contém erros. Não é possível processar.")
            return []
        
        if not self.dfa_accept_state_to_token_type_map:

            if self.dfa.accept_states:
                 self.application.error("Crítico: Mapa de estados de aceitação para tipos de token está vazio, mas DFA tem estados de aceitação.")

            self.application.warning("Aviso: Mapa de estados de aceitação para tipos de token está vazio.")
        ...
        while current_pos < input_len:
            ...
            while scan_pos < input_len:
                ...
                next_dfa_state = self.dfa.transitions.get((current_dfa_state, char))
                ...
                if next_dfa_state is not None:
                    ...
                    if current_dfa_state in self.dfa.accept_states: # dfa.accept_states are frozensets
                        last_accepted_lexeme_info = (current_lexeme_scan, current_dfa_state, scan_pos)
                else:
            ...
            if last_accepted_lexeme_info:
                ...
                base_token_type = self.dfa_accept_state_to_token_type_map.get(final_dfa_accept_state)
                ...
                if not base_token_type:
                    ...
                    tokens.append((final_lexeme, "erro!_TIPO_INTERNO_DESCONHECIDO"))
                    self.application.error(f"Erro Interno: Lexema '{final_lexeme}' aceito pelo estado {final_dfa_accept_state} que não está no mapa de tipos.")
                else:
                    # Check symbol table for reserved words/specific lexemes
                    # This allows "if" (base_token_type 'ID') to become 'PR_IF'
                    overriding_token_type = self.symbol_table.lookup(final_lexeme)
                ...
                if current_pos < input_len: # ter certeza de que ainda há caracteres para processar
                    ...
                    self.application.log(f"Erro Léxico: Caractere inesperado '{error_char}' na posição {current_pos}.")
